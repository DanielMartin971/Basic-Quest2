...
window.addEventListener('load', () => {
  specs.forEach((spec) => {
    if(spec.attributes.name.value == 'type'){
      ...
      spec.innerHTML = Math.floor(self.defense);
    }
    else if(spec.attributes.name.value == 'health'){
      spec.innerHTML = Math.floor(self.maxHealth);
    }
    else if(spec.attributes.name.value == 'magic'){
      spec.innerHTML = Math.floor(self.magic);
    }
    else if(spec.attributes.name.value == 'speed'){
      spec.innerHTML = Math.floor(self.speed);
    }
    else if(spec.attributes.name.value == 'strength'){
      spec.innerHTML = Math.floor(self.strength);
    }
  });
